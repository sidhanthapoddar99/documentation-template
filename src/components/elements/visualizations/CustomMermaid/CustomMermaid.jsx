...
const CustomMermaid = ({ value, title, description }) => {
  ...
  const initializePanzoom = useCallback(() => {
    ...
    // Fix Mermaid container styles
    const containers = wrapperRef.current.querySelectorAll('.docusaurus-mermaid-container, .mermaid');
  ...
  useEffect(() => {
    ...
    // Use a simple polling approach to check for SVG
    let attempts = 0;
