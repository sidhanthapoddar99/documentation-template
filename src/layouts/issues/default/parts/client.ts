...
const FIELDS = ['status', 'priority', 'type', 'component', 'milestone', 'labels'] as const;
...
function apply(cfg: Config) {
  ...
  if (emptyEl) {
    const anyFilter = state.q || FIELDS.some((f) => state.fields[f].size > 0);
