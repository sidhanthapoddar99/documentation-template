...
import { GraphViz, FileGraphViz } from '@site/src/components/elements';
...
- 🎨 **Multiple Layout Engines**: dot, neato, fdp, circo, twopi, osage, patchwork
- 🌓 **Theme Aware**: Automatic color adaptation for light/dark modes
- 🔍 **Full Zoom Control**: Same powerful controls as Mermaid
- 📐 **Precise Layouts**: Mathematical graph layout algorithms
- 🚀 **WebAssembly Performance**: Fast rendering via Viz.js
...
Load graphs from external `.dot` or `.gv` files:

<FileGraphViz 
  filePath="/diagrams/example-graph.dot"
  title="Example Graph"
  description="Loaded from external DOT file"
/>
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `filePath` | `string` | - | **Required.** Path to .dot/.gv file |
