...
Layout components (`.astro` files) contain only HTML structure, data handling, and JavaScript. No CSS.
...
| `layout.css` | Page structure | BaseLayout flow (flex column), content area sizing, responsive structure |
...
@import './reset.css';      /* 4. Browser normalization */
@import './markdown.css';   /* 5. Rendered content styles */
@import './layout.css';     /* 6. Page structure */
...
Breakpoints are **settings** — fixed values that theme CSS files use consistently. They are defined as a convention and documented here so that all theme CSS files use the same values.
