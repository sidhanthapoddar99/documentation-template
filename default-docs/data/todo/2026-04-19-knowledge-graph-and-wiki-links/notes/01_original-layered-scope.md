...
# Original layered scope — conceptual map

The original shape of this issue before it was reframed as an engine restructure. Kept as a design note because the layered breakdown (link resolution / wiki links / graph / surfaces) is still the cleanest way to think about *what* the work delivers, even though *how* it's built is now the URL-registry + unified-pipeline model described in `issue.md` and implemented across `subtasks/01_…` through `07_…`.
...
See `issue.md` for the current engine-restructure framing and `subtasks/01_…` through `07_…` for concrete workstreams.
